...
function SubmitMapVote(int MapIndex, int GameIndex, Actor Voter)
{
	...
	if( !IsValidVote(MapIndex, GameIndex) ) return;
	...
	if( !MapList[MapIndex].bEnabled || (MVRI[Index].MapVote==MapIndex && MVRI[Index].GameVote==GameIndex) )		return;
	...
	if( PrevMapVote > -1 && PrevGameVote > -1 )		UpdateVoteCount(PrevMapVote, PrevGameVote, -MVRI[Index].VoteCount); // undo previous vote
	MVRI[Index].VoteCount = VoteCount;
	TallyVotes(false);
}
// ==========================================================================================
